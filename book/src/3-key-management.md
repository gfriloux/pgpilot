...
2. Find the **Confiance** (Trust) badge — shows current level
   - **Undefined** (gray) — you haven't verified this key
   - **Marginale** (amber) — you've partially verified the owner
   - **Complète** (green) — you've fully verified the owner
   - **Ultime** (green, rare) — you own this key

### Change trust level

1. Select a key in the list
2. In the detail panel, click the **Confiance** badge
3. A picker appears with three options: Undefined, Marginale, Complète
4. Click your choice
...
Select a key in the list. The right panel shows a **Sous-clefs** (Subkeys) section:

```
[S] Signature        Created: 2024-01-15   Expires: Never
    Algo: ed25519  ID: ABC123...

[E] Chiffrement      Created: 2024-01-15   Expires: Never
...
Legend: **[S]** = Signature, **[E]** = Encryption, **[A]** = Authentication.
...
1. Select your key
2. Click **+ Ajouter sous-clef** (Add Subkey)
3. Choose the type: Signature / Encryption / Auth
4. Confirm
...
1. Select a key, then click a subkey card's **Renouveler** button
...
1. Select a key, then click a subkey card's **Remplacer** button
...
1. Select your key
2. Click **Exporter** or the three-dot menu
3. Choose **Fichier** (File)
...
1. Select your key
2. Click **Exporter** or the three-dot menu
3. Choose **Coller** (Paste to paste.rs)
...
7. A status message confirms: "Sauvegarde terminée"
...
3. Select **Fichier** (File)
4. Click **Choisir un fichier** (Choose file)
...
7. Click **Importer** for the key you want
...
3. A confirmation modal appears: "Êtes-vous sûr?"
4. Click **Supprimer**
5. pgpilot calls `gpg --delete-keys <fingerprint>`
6. Key is removed; status shows: "Clef supprimée"
...
1. Select the key
2. Click **Supprimer**
3. A warning modal appears: "Cette action ne peut pas être annulée..."
4. Click **Supprimer la clef secrète**
5. pgpilot calls `gpg --delete-secret-and-public-keys <fingerprint>`
6. Both secret and public key are erased
7. Status: "Clef supprimée"
...
6. If successful, status shows: "Clef publiée sur `<serveur>`"

### Check publication status

1. Select your key
2. In the detail panel, find the **Serveur** (Keyserver) badge:
...
- You can trigger manually anytime by clicking **Publier** again
